...
dir_spatial_results_root_abs = r'D:\Google Drive IU\phdStuff\covid 19 project\ddm results\simple-pk\batch_1'
#dir_spatial_results_rel = [f'set_{x}' for x in range(28)]
dir_root_output_abs = os.path.join(r'D:\Google Drive IU\phdStuff\covid 19 project\ddm results\simple-pk',
                                   'Figs', r'kon_1', 'first_dose_0')
num_runs = 10
output_frequency = 1
screenshot_output_frequency = 50
plot_sets = False
plot_grid = True
plot_infect_calibration = None
#eff_infect_dir_root = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'Effective Infectivity')
eff_infect_dir_root = None
...
dir_spatial_results_rel = get_sets_of_param_value('first_dose', 0, inputs)  # append to this to add the 5th line
...
if plot_infect_calibration is not None:
    def manip_plot_ode(fig, ax):
        ax.lines[-2].set_marker("None")
        ax.lines[-2].set_color('k')
        ax.lines[-1].set_marker("None")
        ax.lines[-1].set_color('gray')
        ax.lines[-1].set_linestyle('--')
    proj_render_defs.render_defs.manip_plot_ode = manip_plot_ode
...
def grid_axes_manip(figfig):
    ...
    figfig.fig.subplots_adjust(top=0.85, bottom=0.15, hspace=0.25)
...
# Displays variations divided by ten, to compensate for execution specification
...
if __name__ == '__main__':
    ...
    for set_idx in range(len(dir_spatial_results_rel)):
        ...
        if eff_infect_dir_root is not None:

            eff_infect_dir_set = os.path.join(eff_infect_dir_root, f"set_{set_idx}")
            batch_data_eff = import_ode_batch_results(eff_infect_dir_set)
            for k in batch_data_eff.keys():
                batch_data_s[k]["eff_infect"] = batch_data_eff[k][0]

            for k in batch_data_eff:
                max_step = max([list(batch_data_s[k][t].keys())[-1] for t in batch_data_s[k].keys() if t != "ODE"])
                batch_data_s[k]["eff_infect"] = {k: v for k, v in batch_data_s[k]["eff_infect"].items() if k < max_step}
        ...
        for sim_run_post in sim_run_post_list:
            for d in sim_run_post.get_data_descs():
                if "ODE" not in sim_run_post.batch_data_summary[d].keys():
                    continue
                run_idx_list = [x for x in sim_run_post.batch_data_summary[d].keys() if x not in ["ODE", "eff_infect"]]
